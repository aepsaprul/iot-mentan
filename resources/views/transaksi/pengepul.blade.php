        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <a href="{{ route('transaksi.create', ['pengepul']) }}" class="btn btn-sm btn-primary" style="width: 130px;"><i class="fas fa-plus"></i> Tambah</a>
            </div>
            <div class="card-body">
              @if (session('message'))
                ...
                <tbody>
                  @foreach ($pengepuls as $key => $pengepul)
                    <tr>
                      ...
                      <td class="p-1">{{ $pengepul->dataPengguna ? $pengepul->dataPengguna->nama : '' }}</td>
                      <td class="p-1">{{ $pengepul->ipfs_hash }}</td>
                      <td class="text-center p-1">
                        <div class="btn-group">
                          <i class="fas fa-cog dropdown-toggle text-primary" data-toggle="dropdown"></i>
                          <ul class="dropdown-menu dropdown-menu-right">
                            <li><a href="{{ route('transaksi.delete', [$pengepul->id]) }}" class="dropdown-item" onclick="return confirm('Yakin akan dihapus?')"><i class="fas fa-trash-alt" style="width: 25px;"></i> Hapus</a></li>
                          </ul>
                        </div>
                      </td>
                    </tr>
