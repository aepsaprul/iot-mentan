@extends('layouts.app')
@section('title') Tambah Pengepul @endsection
@section('style')
<!-- select2 -->
...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container">
      <div class="row">
        <div class="col-sm-6">
          <h1 class="m-0">Tambah Pengepul</h1>
        </div>
        <div class="col-sm-6">
              ...
              <form action="{{ route('pengepul.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                  <div class="col-lg-3 col-12 mb-3">
                    <label for="alamat">Nama Lengkap Pengepul</label>
                    <input type="text" name="nama" id="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama') }}" placeholder="Nama Lengkap Pengepul" required autofocus>
                    @error('nama')
                  ...
                  <div class="col-lg-3 col-12 mb-3">
                    ...
                    <input type="text" name="telepon" id="telepon" class="form-control @error('telepon') is-invalid @enderror" value="{{ old('telepon') }}" placeholder="Telepon / WhatsApp" required autofocus>
                    @error('telepon')
                      <span class="text-danger text-sm font-italic">{{ $message }}</span>
                    @enderror
                  </div>
                  <div class="col-lg-3 col-12 mb-3">
                    ...
                    <input type="text" name="email" id="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="Email" required autofocus>
                    @error('email')
                      <span class="text-danger text-sm font-italic">{{ $message }}</span>
                    @enderror
                  </div>
                  <div class="col-lg-3 col-12 mb-3">
                    ...
                    <input type="text" name="password" id="password" class="form-control @error('password') is-invalid @enderror" value="{{ old('password') }}" placeholder="Password" required autofocus>
                    @error('password')
                      <span class="text-danger text-sm font-italic">{{ $message }}</span>
                    @enderror
                  </div>
